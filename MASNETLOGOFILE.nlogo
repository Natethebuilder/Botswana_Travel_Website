...
to setup-environment
  if environmentType = 0 [ ; Fully Segregated (no gray square)
...
to move-within-region
  ...
  ;; Check if the agent is outside their assigned region
  if pcolor != region-color [
    ;; Move the agent back to a random patch within their region
    move-to one-of patches with [pcolor = region-color]
  ]

  ;; Allow small random movement within the region
  rt random 90
  fd 1

  ;; Ensure agents do not move outside their region accidentally
  if pcolor != region-color [
    move-to one-of patches with [pcolor = region-color]
  ]
end
...
to interact
  ...
  if neighbor != nobody [
    ...
    ;; Check if all forced interventions are inactive
    let forced-interventions-inactive? (
      youthSocializationIntensity = 0 and
      sharedResourceIntensity = 0 and
      communityEventIntensity = 0
    )

    ;; Logic for environment 0
    if environmentType = 0 [
      ;; Prevent inter-group interactions if forced interventions are inactive
      if group != interaction-group and forced-interventions-inactive? [
        stop ;; No interaction or defection
      ]
    ]

    ;; Logic for environment 1 and 2 (includes other interventions)
    if environmentType != 0 [
      ;; Calculate the probability of inter-group cooperation
      let inter-group-cooperation-probability (
        (legalPolicyIntensity / 100 * 0.2) +  ;; Legal policies reduce barriers
        (trainingIntensity / 100 * 0.3) +    ;; Training promotes cooperation
        (economicIncentiveIntensity / 100 * ifelse-value (economic-status < 50) [0.2] [0]) +
        (sharedResourceIntensity / 100 * 0.2) + ;; Shared resources have a minor effect
        (communityEventIntensity / 100 * 0.2)  ;; Community events build trust
      )
      ;; Promote cooperation if probability threshold is met
      if group != interaction-group and random-float 100 < inter-group-cooperation-probability * 100 [
        set interaction-outcome 1 ;; Cooperation
      ]
    ]

    ;; Determine forced interaction logic for all environments
    let forced-interaction? (
      (youthSocializationIntensity > 0 and age < 18) or
      (sharedResourceIntensity > 0) or
      (communityEventIntensity > 0)
    )

    ;; Default intra-group interaction
    if group = interaction-group [
      ;; Intra-group interaction
      let intra-group-defection-probability 10 ;; 10% baseline defection
      if random-float 100 > intra-group-defection-probability [
        set interaction-outcome 1 ;; Cooperation
      ]
    ]

    ;; Inter-group interaction for environments 1 and 2 with or without forced interventions
    if group != interaction-group and (forced-interaction? or environmentType != 0) [
      if random-float 100 > prejudice [
        set interaction-outcome 1 ;; Cooperation
      ]
    ]

    ;; Update statistics and prejudice
    if interaction-outcome = 1 [
      ...
      set prejudice prejudice - 2 ;; Cooperation reduces prejudice
    ]
    if interaction-outcome = 0 [
      if group = interaction-group [ set defown defown + 1 ]
      if group != interaction-group [ set defother defother + 1 ]
      set prejudice prejudice + 1 ;; Defection increases prejudice
    ]

    ;; Cap prejudice between 0 and 100
...
to update-prejudice
  ask people [
    ; Natural prejudice increase
    if random-float 100 < 20 [ ; 20% chance per tick for natural prejudice growth
      set prejudice prejudice + 0.5
    ]

    ; Effects of interventions
    let intervention-effect
      (legalPolicyIntensity / 100 * 0.2) +
      (trainingIntensity / 100 * 0.3) +
      (communityEventIntensity / 100 * (count turtles in-radius 3 / 5)) +
      (economicIncentiveIntensity / 100 * ifelse-value (economic-status < 50) [1] [0]) +
      (sharedResourceIntensity / 100 * 0.2) +
      (youthSocializationIntensity / 100 * ifelse-value (age < 18) [1] [0])

    set prejudice prejudice * (1 - intervention-effect)

    ; Cap prejudice to [0, 100]
    set prejudice max list 0 (min list 100 prejudice)
  ]
end
